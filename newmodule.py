    ...
    for nodes in node_pair:
        ...
        if not nx.has_path(G, nodes[0], nodes[1]):
            ...
            for i in range(len(path) - 1):
                ...
                mini_traffic[link_num].append(nodes)
...
def get_key_index(keys, fail_set):
    return fail_set.index(keys)

def calcul_load(traffic, phy_elist, logic_elist):
    mini_traffic = {}
    global node_pair
    global fail_set
    keys_set = (set(phy_elist) - set(logic_elist))
    keys = [i for i in keys_set]
    if keys in fail_set:
        index = fail_set.index(keys)
        mini_traffic = traffic[index].copy()
    else:
        fail_set.append(keys)
        for edge in phy_elist:
            mini_traffic[edge] = []
        ...
        for nodes in node_pair:
            ...
            if not nx.has_path(G, nodes[0], nodes[1]):
                ...
                for i in range(len(path) - 1):
                    link_num = (path[i], path[i + 1]) if path[i] <= path[i + 1] else (path[i + 1], path[i])
                    mini_traffic[link_num].append(nodes)
        traffic[fail_set.index(keys)] = mini_traffic

    load = [0 for i in range(len(phy_elist))]
    i = 0
    for x in mini_traffic:
        load[i] = len(mini_traffic[x])
        i += 1
...
class Link(object):
    ...
    def update_link(self, timestep, traffic, phy_elist, logic_elist, load):
        ...
        if self.readyDown == 1 and self.holdTime_Timer >= HOLD_TIME:
            ...
            #   更新edge表
            self.link_state = -1
            if self.link_num in logic_elist:
                logic_elist.remove(self.link_num)
        ...
        if self.readyStart == 1 and self.reStart_Timer == en.reStart_Timer and not self.manual_damage:
            ...
            #   更新edge表
            self.link_state = 1
            if self.link_num not in logic_elist:
                logic_elist.append(self.link_num)
...
def init_network(phy_elist, load):
    ...
    # logout("initialize successfully!!!")
...
def begin_cicle(traffic, phy_elist, logic_elist, load, action, run_time,links):
    ...
    while(timestep < run_time):
        ...
        #print(timestep)
        if timestep == 0:
            #for action_num in action:
            damage_link = links[phy_elist.index(action)]
            damage_link.readyDown = 1
            damage_link.manual_damage = 1
        for link in links:
            if link.link_num[0] != link.link_num[1]:
                link.update_link(timestep, traffic, phy_elist, logic_elist, load)

        traffic, load = calcul_load(traffic, phy_elist, logic_elist)
        #更新每个link对应的load
        for link in links:
            link.link_now_load = load[phy_elist.index(link.link_num)]
...
def create_attackList(phy_elist):
    ...
    # logout("chose these link:", action_list)
...
if __name__ == '__main__':
    ...
    traffic[0] = cacul_traffic(logic_elist)
    fail_set.append([])
    ...
    #   失效链路选择
    action_list = create_attackList(phy_elist)

    #   开始时间循环
    topology, faillink_num, loadsum = begin_cicle(traffic, phy_elist, logic_elist=logic_elist, load=load, action=action_list, run_time=1000, links=links)
    ...
    y_axis_data = [0] + faillink_num[:1000-1]
